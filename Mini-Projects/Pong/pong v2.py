...
class Game:
   ...
   def __init__(self, surface):
      ...
      self.small_dot = Dot('green', 5, [50, 50], [2,1], self.surface)
      self.paddle = Paddle('red',0,0,5,30,self.surface)
      #self.max_frames = 150
   ...
   def handle_events(self):
      ...
      for event in events:
         ...
         if event.type == pygame.KEYDOWN:
            self.paddle.move_start(event)
         if event.type == pygame.KEYUP:
            self.paddle.move_stop(event)
...
class Dot:
   ...
   def move(self):
      ...
      if self.center[1] <= self.radius or self.center[1] >= height:
...
class Paddle:
   ...
   def __init__(self, rect_color, horiz_pos, vert_pos, rect_width, rect_height, surface):
